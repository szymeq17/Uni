...
(define (builtin/1 p)
  (builtin (lambda (x) (list (cons (car x) (p (cdr x))))) null 1))
(define (builtin/2 p)
  (builtin (lambda (x y) (list (cons (* (car x) (car y)) (p (cdr x) (cdr y))))) null 2))
...
;; Definicja procedury uniform  <------------------------------- new
(define (uniform xs)
  (define (go xs n)
    (if (null? xs)
        null
...
(define prob-env
  (foldl (lambda (p env) (env-add (first p) (second p) env))
         start-env
         `((flip ,(effect-builtin/3 (lambda (p x y) (list (cons (cdr p) (cdr x))
                                                          (cons (- 1 (cdr p)) (cdr y))))))
           )))

(define (concat-map xs f) (append-map f xs))

(define (make-pairs n pairs)
  (if (null? pairs)
      null
      (cons (cons (* n (caar pairs))
                  (cdar pairs))
            (make-pairs n (cdr pairs)))))
...
(define (eval-env e env)
  (match e
    ...
    [(var-expr x)
     (list (cons 1 (let ([e (env-lookup x env)])
                         (if (cons? e)
                             (cdar e)
                             e))))]
    ...
    [(if-expr eb et ef)
     ...
     (match (cdr vb)
       [#t   (let ([t (eval-env et env)])
               (make-pairs (car vb) t))]
       [#f   (let ([f (eval-env ef env)])
               (make-pairs (car vb) f))]
       [v    (error "Not a boolean:" v)])))]

    [(lambda-expr x e)
     (list (cons 1 (clo x e env)))]
       ...
       (match (cdr vf)
         ...
         [(builtin p args nm)
          (if (= nm 1)
              ...
              (list (cons 1 (builtin p (cons va args) (- nm 1)))))]
...
(eval (parse'(flip  0.5 (lambda (x) (+ 1 x)) (lambda (x)(+ 1 x)))))