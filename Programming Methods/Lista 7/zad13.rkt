...
(define (free e)
    (let ([bound (bound-vars e null)])
          (free-vars e null bound)))
...
(define (find-dead-vars e)
    (define (walk e vars)
        (match e
            [(const n) (const n)]
            [(binop op l r) 
             (binop op (walk l (append vars (free r)))
                       (walk r vars))]
            [(let-expr id e1 e2) 
             (let-expr id (walk e1 vars) (walk e2 vars))]
            [(var-expr id)
...
;(parse '(let [x 5] (* (+ x 5) (let [x 4] (+ x 3)))))
(find-dead-vars (parse '(let [x 5] (* (+ (+ x 5) (- x 2)) (let [x 4] (+ x 3)))))) 
(find-dead-vars
(let-expr 'x (const 5)
             (binop '* (binop '+ (binop '+ (var-expr 'x) (const 5))
                                 (binop '- (var-expr 'x) (const 2)))
                       (let-expr 'x (const 5)
                                    (binop '* (binop '+ (binop '+ (var-expr 'x) 
                                                                  (const 5))
                                                        (binop '- (var-expr 'x) (const 2))) 
                                              (var-expr 'x))))))
