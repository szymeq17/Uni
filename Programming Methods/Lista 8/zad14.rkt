...
(struct app-list (f e)        #:transparent) ; nowe
...
(define (expr? e)
  (match e
    ...
    [(app-list f e) (and (expr? f) (expr? e))] ; nowe
...
(define (parse q)
  (cond
    ...
    [(and (list? q) (eq? (length q) 3) (eq? (first q) 'apply)  )  ;;dodane
     (app-list (parse (second q)) (parse (third q)))]  ;; dodane
...
(define (changer q xs)
  (if (null? xs)
      q
      (changer (app q (parse (car xs))) (cdr xs)))) 

;Testy:
(eval (parse '(apply (lambda (x y) (/ x (car (cons 1 2)))) (cons 1 (cons 2 null)))))
;(eval (parse '(apply (lambda (x) (+ x 1)) (cons 1 null))))
;(eval (parse '(let [y 4] (let [y 2] (apply (lambda (x) x) (list y))))))
;(eval (parse '(apply (lambda (x y z) (+ x (+ y z))) (list 1 2))))
;(eval (parse '(apply (lambda (x) (+ 1 x)) (list))))
;(eval (parse '(apply (lambda (x y) (lambda (z) (+ x (+ y z)))) (list 1 2 3 4))))
;(eval (parse '(apply (lambda (x y) (+ y x)) (list))))
