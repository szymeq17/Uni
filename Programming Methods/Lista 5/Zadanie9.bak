...
(define (falsifiable-cnf? f)
  (let* ([nnf-formula (to-nnf f)]
    ...
    (define (falsifiable-cnf-helper f)
      (if (null? f)
          #f
          (if (eq? #f (falsifiable-clause? (car f)))
              (falsifiable-cnf-helper (cdr f))
              (falsifiable-clause? (car f)))))
...
;---------------------------------------------------------------------------
#|
Funkcja falsifiable-cnf? daje wynik znacząco szybciej, ponieważ ma złożonosć
kwadratową/sześcienną. W najbardziej pesymistycznym przypadku formułą w CNF
