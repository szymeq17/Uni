...
(define (lcdr xs) (if (procedure? (mcdr xs))
                      ((mcdr xs))
                      (mcdr xs)))
(define (set-lcar! xs v) (set-mcar! xs v))
(define (set-lcdr! xs v) (set-mcdr! xs v))



;Procedura lnth wylicza n-ty element listy
;i spamiętuje wszystkie, które po drodze oblicza
;=================================================
(define (lnth n xs)
  (cond [(= n 0) (lcar xs)]
        [else (begin (set-lcdr! xs (lcdr xs))
                     (lnth (- n 1) (lcdr xs)))]))
;=================================================
