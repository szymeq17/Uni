...
;eval dostaje teraz także środowisko z while
;i jeśli nie znajdzie tego czego szuka w swoim środowisku
;to szuka w środowisku z while
(define (eval-env e env while-env)
  (match e
    [(const n) n]
    [(binop op l r) ((op->proc op) (eval-env l env while-env)
                                   (eval-env r env while-env))]
    [(let-expr x e1 e2)
     (eval-env e2 (env-add x (eval-env e1 env while-env) env) while-env)]
    [(letrec-expr x e1 e2)
    ...
    [(var-expr x) (let ([look (env-lookup x env)])
                    (if (eq? look #f)
                        (env-lookup x while-env)
                        look))]
    [(if-expr eb et ef) (if (eval-env eb env while-env)
                            (eval-env et env while-env)
                            (eval-env ef env while-env))]
    [(cons-expr e1 e2) (cons (eval-env e1 env while-env)
                             (eval-env e2 env while-env))]
    [(car-expr e) (car (eval-env e env while-env))]
    [(cdr-expr e) (cdr (eval-env e env while-env))]
    [(null-expr) null]
    [(null?-expr e) (null? (eval-env e env while-env))]
    [(lam x e) (clo x e env)]
    [(app f e)
     (let ([vf (eval-env f env while-env)]
           [ve (eval-env e env while-env)])
...
(env-lookup 'z (eval-while (parse-while P1) env-empty))
(env-lookup 'z (eval-while (parse-while P2) env-empty))
(env-lookup 'z (eval-while (parse-while P3) env-empty))
(env-lookup 'z (eval-while (parse-while P4) env-empty))
(fact 4)
(eval-while (parse-while P1) env-empty)
